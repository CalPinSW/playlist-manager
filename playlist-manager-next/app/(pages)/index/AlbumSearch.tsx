...
import { createUndefinedArray } from "../../utils/createUndefinedArray";
import AlbumSlide from "../../../components/carousel/AlbumSlide";

interface AlbumWithPlaylists extends album {
...
const [albumSearch, setAlbumSearch] = useState<string>("")
    ...
    const data = playlistQuery.data as AlbumWithPlaylists[] ?? createUndefinedArray(pagination.pageSize);
    return ( 
        <div className="flex flex-col gap-4">
        <SearchBar title="Album Search" search={albumSearch} setSearch={setAlbumSearch}/>
        <Carousel slides={
            data.map(
            (p, index) =><AlbumSlide album={p} key={`albumSearch ${index}`}/>
            )} 
        />  
        </div>
    );
...
export const albumSearchQueryOptions = (pagination: PaginationState, albumSearch: string ) => queryOptions({
