...
export default async function Index() {
  ...
  if (playlist?.name.startsWith('New Albums')) {
    fetchPlaylists(playlist.name.slice(11)).then(associated => {
      setAssociatedPlaylists(associated.filter(p => p.name !== playlist.name));
    });
  }
  ...
  if (playlist?.name.slice(0, 10) === 'New Albums') {
    fetchPlaylists(playlist.name.slice(11)).then((associatedPlaylists: playlist[]) => {
      setAssociatedPlaylists(
        associatedPlaylists.filter(associatedPlaylist => associatedPlaylist.name !== playlist.name)
      );
    });
  }
  ...
  return (
    <div className="p-2 text-sm sm:text-base space-y-4">
        ...
        <div className="mt-2">
          <div className="mt-2">
            <button
              ...
              onClick={() => setViewMode(viewMode === ViewMode.ALBUM ? ViewMode.TRACK : ViewMode.ALBUM)}
            >
              <h2
                className={`p-2 flex grow text-right ${viewMode === ViewMode.ALBUM ? 'bg-primary-darker' : ''} ${
                  !playlistAlbums ? 'opacity-50 disabled' : ''
                }`}
              >
                {`Album View (${playlistAlbums?.length ?? 0})`}
              </h2>
              <h2 className={`p-2 flex grow ${viewMode === ViewMode.TRACK ? 'bg-primary-darker' : ''}`}>
      ...
      <ToastContainer />
      {playlistAlbums && <button onClick={() => void copyAlbumArtistList(playlistAlbums)}>Copy Album List</button>}
    </div>
  );
}
