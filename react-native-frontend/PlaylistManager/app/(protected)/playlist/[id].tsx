...
const PlaylistExplorer: React.FC = () => {
		...
		if (activeAlbumId === albumId) {
			...
			if (carouselRef.current) {
				carouselRef.current.scrollTo({
					index: playlistAlbums?.findIndex(
	...
	const activeAlbum = playlistAlbums?.find(
		album => album.id === activeAlbumId,
	);
			...
			<View style={{ flex: 1, alignSelf: "flex-start" }}>
				<Carousel
					...
					renderItem={album => (
						<AlbumSlide
							album={album}
							isSelected={activeAlbumId == album.id}
							onPress={() => onAlbumSelect(album.id)}
						/>
					)}
				/>
				{activeAlbum && (
